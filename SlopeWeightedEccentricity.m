...
%  2. A Laplacian-of-Gaussian (LoG) mask created from the bathymetry using 
%     a third party software/tool in *.xyz format
%     (here: 'Input_LoG_mask.xyz')

%  If the LoG mask is not going to be used, we suggest creating a grid with
%  the size and region of the gridded shipborne multibeam bathymetry and
%  assigning the number '1' to all the cells (a 'no mask' example named
%  'Input_no_mask.xyz' is provided) OR by exempting all the lines with
...
% INPUT: Laplacian-of-Gaussian (LoG) mask from a third-party software/tool
% The resolution must comply with the determined 'cellsize'

mask_input=load('Input_no_mask.xyz'); 
...
title ('e'); axis equal % of > 20 slope (\circ)')
...
ylim([min(min(lat_rs)) max(max(lat_rs))])
set(gca,'FontSize',16); ytickangle(90)
title ('sin(slope) as weight (W)'); axis equal % of > 20 slope (\circ)')
...
ylim([min(min(lat_rs)) max(max(lat_rs))])
set(gca,'FontSize',16); ytickangle(90)
title ('e * W_(_f_r_o_m_ _S_l_o_p_e_)'); axis equal % of > 20 slope (\circ)')

%% Plot e * weight in masked area ( LoG > 0 )
...
ylim([min(min(lat_rs)) max(max(lat_rs))])
set(gca,'FontSize',16); ytickangle(90)
title ('e * W_(_f_r_o_m_ _S_l_o_p_e_) at Positive LoG'); axis equal % of > 20 slope (\circ)')
